...
class SubmissionFileController extends Controller
{
    public function store(Request $request, $activityId)
    {
        ...
        foreach ($files as $file) {
            ...
            $filePath = $file->store('submission_files', 's3'); // Store the file in the 's3' disk and get the path
    ...
    public function checkSubmission($activityId)
    {
        ...
        if ($submission) {
            $submissionFiles = DB::table('submission_files')
                ...
                ->get()
                ->map(function ($file) {
                    $file->file_path = Storage::disk('s3')->url($file->file_path);
                    return $file;
                });
    ...
    public function updateSubmissionFiles($submissionId, Request $request)
    {
        ...
        foreach ($files as $file) {
            $originalName = $file->getClientOriginalName();
            $filePath = $file->store('submission_files', 's3');
            $fileType = $file->getClientOriginalExtension();
            $fileSize = $file->getSize();
